q.scoreplot.ord <- function(results, factor, extreme.labels = c("negative", "positive"), incl.qdc = TRUE, quietly = FALSE, hyph.pattern = "en", hyphenate = TRUE, split.string = "[[:punct:]]", label.scale=500) {
  # Input verification ===================
  if (!is.null(factor)) {
  ...
  current.fac <- factor  # this is because below code used to be in another forloop
  current.fac.n <- factors[names(factors) == current.fac]
  array.viz.data <- cbind(  # read in data
    results$zsc_n[current.fac.n]  # the x position, rounded (zscore would be nicer, but is hard to visualize because of overlapping item labels etc.)
    , results$zsc[current.fac.n]  # also add zscore, maybe for future use
    , q.item.sd(results = results)[current.fac.n]  # get sd per item, call other function
  )
  ...
  if (incl.qdc == TRUE) {
    ...
    current.fac.n <- factors[names(factors) == current.fac]
    for (other.fac in factors[names(factors) != current.fac]) {
      sig2other <- as.matrix(results$qdc[rownames(array.viz.qdc.diff), grepl(names(factors)[current.fac.n], colnames(results$qdc)) & grepl(names(factors)[other.fac], colnames(results$qdc)) & grepl("sig", colnames(results$qdc))])
      ...
      diff2other <- as.matrix(results$qdc[rownames(array.viz.qdc.diff), grepl(names(factors)[current.fac.n], colnames(results$qdc)) & grepl(names(factors)[other.fac], colnames(results$qdc)) & !grepl("sig", colnames(results$qdc))])
...
#   g.int <- ggplot(data = array.viz.data, mapping = aes(x = 0, y = zsc, label = rownames(array.viz.data)))
#   array.viz.data
#   array.viz.data$hjust <- rep_len(x = c(1,0), length.out = 77)[order(array.viz.data$zsc)]
#   hist(array.viz.data$zsc)
#   hist(results$zsc$critical, breaks = c(-7:7))
#   hist(results$zsc$resentment)
#   hist(results$zsc$moderate)
#   norm_trans <- function() {
#     trans_new('norm', function(x) pnorm(x), function(x) qnorm(x))
#     # this is from http://stackoverflow.com/questions/12186827/transform-color-scale-but-keep-a-nice-legend-with-ggplot2
#   }
#   library(scales)
#   zsc.max <- max(abs(array.viz.data$zsc))
#   array.viz.data$ranks.scaled <- (rank(x = array.viz.data$zsc, ties.method = "first")/nrow(array.viz.data) - 0.5) * zsc.max * 2
#     help(rank)
#   g.int <- g.int + geom_point()
#   g.int <- g.int + xlim(c(-1,1))
#   g.int <- g.int + ylim(-zsc.max, zsc.max)
#   g.int <- g.int + geom_text(mapping = aes(x = -0.5, y = array.viz.data$ranks.scaled), hjust = 1)
#   g.int + geom_line(mapping = )
#   g.int + geom_abline()
#
#   array.viz.data <- array.viz.data[rank(x = array.viz.data$zsc, ties.method = "first"),]
#   g.int <- ggplot(data = array.viz.data, mapping = aes(x = zsc, y = reorder(rownames(array.viz.data), zsc)))
#   g.int
#   g.int + geom_point()
#
#   g.int <- ggplot(data = dattest, mapping = aes(color = factor, x = zsc, y = reorder(rownames(dattest), dattest[dattest$factor=="resentment", "zsc"]))) + geom_point()
#
#   g.int
#
#   g.int <- ggplot(data = dattest, mapping = aes(color = factor, x = zsc, y = reorder(rownames(dattest), dattest[dattest$factor=="resentment", "zsc"]))) + geom_point()
#
#   dattest <- melt(data = results$zsc, variable.name = "factor", value.name = "zsc")
#
#   g.int + geom_text(mapping = aes(#+ geom_errorbar(array.viz.data$item.sd) #+ scale_y_continuous(trans = 'norm') # + coord_trans(y = scales::probability_trans(distribution = 'norm')) # + scale_y_log10()
#   array.viz.qdc
#   array.viz.data$item.sd
#   # + coord_trans(ytrans = 'norm')
#   geom_text(mapping = aes(x = -0.5, y = rank(zsc)), hjust = 1)
#   head(array.viz.data)
#   array.viz.data$full.label <- row.names(array.)
#
#   array.viz.data
#
#   g.list$resentment
