import.q.concourse <- function(q.concourse.dir, languages=c("english")) {
  ...
  for (lang in languages) {
    lang <- list.files(path = paste(q.concourse.dir, "/", lang, sep=""),
                              no.. = TRUE, #  no dotfiles
  ...
  message(paste("Gathered", length(q.concourse)/length(languages), "full items, each in", length(languages), "languages."))
  return(q.concourse)
}
