q.item.sd <- function(qmethodresults) {
	# calculates the sd of an item on a factor, weighted by loadings

  # Input validation
  if (class(qmethodresults) != "QmethodRes") {
  ...
  wraw_all <- qfwe(  # take all objects from existing qmethod result
    dataset = qmethodresults$dataset,
    loa = qmethodresults$loa, 
    flagged = qmethodresults$flagged
  )  # notice that all of the below depends on a SPECIFIC qmethod result, hence the importing
  item_sd <- data.frame(cbind(rep(x = NA, qmethodresults$brief$nstat)))
    # make empty df
  rownames(item_sd) <- row.names(qmethodresults$dataset)
  for (i in 1:qmethodresults$brief$nfactors) {  # loop over the number of factors
  ...
  if (colnames(results$loa)[1] == "f1") { # hack to figure out if results object has been q.fnamed()
    colnames(item_sd) <- paste("item_sd_", c(1:qmethodresults$brief$nfactors), sep="")  # number if unnambed
  } else {
