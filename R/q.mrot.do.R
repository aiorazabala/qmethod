q.mrot.do <- function(results, rot.mat, quietly = FALSE) {  # this function simply implements some rotation matrix
  ...
  if (all(rot.mat == diag(nrow(rot.mat)))) {
    ...
    if (is.null(dimnames(rot.mat))) {  # if there are NO dimnames
      colnames(rot.mat) <- paste0("mrot.f", 1:results$brief$nfactors)  # names must STILL change, because rotations change the context!
      rownames(rot.mat) <- paste0("mrot.f", 1:results$brief$nfactors)  # names must STILL change, because rotations change the context!
